...
bool SelectStep2Scene::init(){
    ...
    textFieldContent = EditBox::create(Size(visibleSize.width-80,300), Scale9Sprite::create("alpha.png"));
...
void SelectStep2Scene::pushDataToNetWork(){
    ...
    string content="";
    if (strcmp("", textFieldContent->getText())) {
        log("length:%s",textFieldContent->getText());
        content=textFieldContent->getText();
    }
