...
bool SelectStep3Scene::init(){
    ...
    webView->setContentSize(Size(visibleSize.width, visibleSize.height-150));
 ...
 /*   auto cameraBtn=Button::create("btn_step3_camera.png");
    ...
    albumBtn->addTouchEventListener([&](Ref* sender, cocos2d::ui::Widget::TouchEventType type){ switch (type){
        case ui::Widget::TouchEventType::BEGAN: break;
        case ui::Widget::TouchEventType::ENDED:{
         TakePhoto::getInstance()->openPhoto([=](std::string path)
                                                {
                                                    CCLOG("photo path:%s",path.c_str());
                 });
     auto chatSC=ChatViewScene::create();
...
Layer* SelectStep3Scene::createMessageLayer(){
    auto visibleSize=Director::getInstance()->getVisibleSize();
    Vec2 origin=Director::getInstance()->getVisibleOrigin();
    auto layer = LayerColor::create(Color4B(50, 50, 50, 255/3));
    ...
    sureBtn->addTouchEventListener([&](Ref* sender, cocos2d::ui::Widget::TouchEventType type){ switch (type){
        case ui::Widget::TouchEventType::BEGAN: break;
        case ui::Widget::TouchEventType::ENDED:{
            TakePhoto::getInstance()->openCamera([=](std::string path)
                                                 {
                                                     CCLOG("photo path:%s",path.c_str());
                                                 });
