...
EditBox*  AddVisitCaseScene::createBasicData(Sprite* bkView,Vec2 point,string name1,string name2){
...
void AddVisitCaseScene::createCaseToNetWork(){
    ...
    char memberUrl[1000]={0};
    sprintf(memberUrl,"patientId=%s&doctorId=%s&type=%s&status=%s%s",userData["data"]["id"].GetString(),UserDefault::getInstance()->getStringForKey("id").c_str(),"followed","new",keyValue.c_str());
    char* url=memberUrl;
