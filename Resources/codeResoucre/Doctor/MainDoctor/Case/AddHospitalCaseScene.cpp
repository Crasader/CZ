...
using namespace std;
USING_NS_CC;
Scene *AddHospitalCaseScene::createScene(){
    return AddHospitalCaseScene::create();
}
bool AddHospitalCaseScene::init(){
    if (!Scene::init()) {
        return false;
    }
    Size visibleSize=Director::getInstance()->getVisibleSize();
    auto bkView=Sprite::create("bk_add_hospital.png");
...
//            Director::getInstance()->pushScene(sickroomSC);
            if (strcmp(textfieldName->getText(), "")!=0&&strcmp(textfieldAge->getText(), "")!=0&&strcmp(textfieldBedNum->getText(), "")!=0&&strcmp(sexLB.c_str(), "")!=0) {
    ...
    importCompleteBtn->setPosition(Vec2(visibleSize.width/2, 400));
    ...
    manCheckBox->addEventListener(CC_CALLBACK_2(AddHospitalCaseScene::checkBoxCallback,this));
    ...
    womanCheckBox->addEventListener(CC_CALLBACK_2(AddHospitalCaseScene::checkBoxCallback,this));
    //获取checkbox的选中状态
    ...
    textfieldCase=createBasicData(bkView, Vec2(59, 640), "病案号：", "未填写");
     textfieldCase->setInputMode(cocos2d::ui::EditBox::InputMode::NUMERIC);
    textfieldBedNum=createBasicData(bkView, Vec2(59, 550), "床号：", "未填写");
     textfieldBedNum->setInputMode(cocos2d::ui::EditBox::InputMode::NUMERIC);
    
    return true;
}

EditBox*  AddHospitalCaseScene::createBasicData(Sprite* bkView,Vec2 point,string name1,string name2){
...
//实现CheckBox回调函数
void AddHospitalCaseScene::checkBoxCallback(cocos2d::Ref * ref, CheckBox::EventType type)
{
...
void AddHospitalCaseScene::eventCallBack(Ref* pSender,cocos2d::ui::TextField::EventType type)
{
    switch (type){
...
void AddHospitalCaseScene::getUserInfoToNetWork(){
    ...
    char memberUrl[1000]={0};
    sprintf(memberUrl,"userId=%s&passwd=%s&name=%s&sex=%s&age=%s&caseNo=%s&number=%s",textfieldName->getText(),"123456",textfieldName->getText(),gender.c_str(),textfieldAge->getText(),getTimeForSystem().c_str(),textfieldBedNum->getText());
    char* url=memberUrl;
