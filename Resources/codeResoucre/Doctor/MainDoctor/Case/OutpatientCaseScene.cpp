...
using namespace std;
USING_NS_CC;
Scene *OutpatientCaseScene::createScene(){
    return OutpatientCaseScene::create();
}
bool OutpatientCaseScene::init(){
    if (!Scene::init()) {
        return false;
    }
    Size visibleSize=Director::getInstance()->getVisibleSize();
    auto bkView=Sprite::create("bk_outpatient_case.png");
    ...
    lv->addEventListener((ui::ListView::ccListViewCallback)CC_CALLBACK_2(OutpatientCaseScene::selectedItemEvent, this));//添加监听函数
    lv->addEventListener((ui::ListView::ccScrollViewCallback)CC_CALLBACK_2(OutpatientCaseScene::selectedItemEventScrollView, this));
    this->addChild(lv);
    return true;
}

void OutpatientCaseScene::onEnter(){
    Scene::onEnter();
    Size visibleSize=Director::getInstance()->getVisibleSize();
    lv->removeAllItems();
...
/*    auto textFieldContent= TextField::create("一百字以内","Arial",35);
    ...
    textFieldContent->setTextHorizontalAlignment(TextHAlignment::LEFT);
    textFieldContent->setTag(110);
    textFieldContent->addEventListener(CC_CALLBACK_2(OutpatientCaseScene::eventCallBack, this));   */
...
void OutpatientCaseScene::createTextFieldView(Layout* contentV){
    Size visibleSize=Director::getInstance()->getVisibleSize();
    auto textFieldBottom=Sprite::create("textfield_bottom.png");
...
Layout *OutpatientCaseScene::createMessageLayout(int i,string title,string content){
...
void OutpatientCaseScene::selectedItemEventScrollView(Ref* pSender, ui::ScrollView::EventType type)
{
    switch (type) {
...
void OutpatientCaseScene::eventCallBack(Ref* pSender,cocos2d::ui::TextField::EventType type)
{
    auto visibleSize=Director::getInstance()->getVisibleSize();
...
Size OutpatientCaseScene::calculateFontSize(const char *str )
{
    auto visibleSize=Director::getInstance()->getVisibleSize();
...
#pragma-个人资料界面
void OutpatientCaseScene::getUserDataToNetWork(){
    ...
    netManeger->sendMessage(memberURL,CC_CALLBACK_2(OutpatientCaseScene::onHttpRequestCompleted2, this),nullptr);
}

void OutpatientCaseScene::onHttpRequestCompleted2(HttpClient* sender, HttpResponse* response)
{
    auto visibleSize=Director::getInstance()->getVisibleSize();
    if (!response)
    {
...
#pragma-网络数据
void OutpatientCaseScene::getCaseInfoToNetWork(){
    NetWorkManger* netManeger =NetWorkManger::sharedWorkManger();
    char memberUrl[500]={0};
    sprintf(memberUrl,"http://czapi.looper.pro/web/getCaseById?caseId=%s",UserDefault::getInstance()->getStringForKey("caseId").c_str());
    netManeger->postHttpRequest(memberUrl,CC_CALLBACK_2(OutpatientCaseScene::onHttpRequestCompleted, this),nullptr);
}

void OutpatientCaseScene::onHttpRequestCompleted(HttpClient* sender, HttpResponse* response)
{
    auto visibleSize=Director::getInstance()->getVisibleSize();
    if (!response)
    {
...
#pragma-获取网络图片
void OutpatientCaseScene::getImageFromNetWork(std::string url,int photoTag){
    NetWorkManger* netManeger =NetWorkManger::sharedWorkManger();
    netManeger->tag=to_string(photoTag);
    netManeger->upLoadData(url.c_str(),CC_CALLBACK_2(OutpatientCaseScene::onHttpRequestImageCompleted, this),nullptr);
}
void OutpatientCaseScene::onHttpRequestImageCompleted(cocos2d::network::HttpClient* sender, cocos2d::network::HttpResponse* response)
{
    auto visibleSize=Director::getInstance()->getVisibleSize();
    char tagStr[50];
    if (!response)
...
float OutpatientCaseScene::createVideoLayerForListView(float height,Layout*bkView){
    auto visibleSize=Director::getInstance()->getVisibleSize();
...
std::string OutpatientCaseScene::searchContentForInfoData(std::string content,std::string title,rapidjson::Document& data){
