...
void TreatScene::onEnter(){
    Size visibleSize=Director::getInstance()->getVisibleSize();
    Scene::onEnter();
    if(isBlueViewMenu==1){//当blueV张开或者收拢时，不刷新界面
        isBlueViewMenu=0;
    }else{
        lv->removeAllItems();
...
//后路
    ...
    if (height3!=0) {
#pragma-如果有数据就加入到listView中
        auto layout=layoutDic.at(tag);
        lv->insertCustomItem(layout, 1);
    }
