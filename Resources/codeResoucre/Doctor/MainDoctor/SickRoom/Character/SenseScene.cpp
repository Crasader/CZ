...
#pragma-洋葱人
    ...
    confirmBtn->addTouchEventListener([&](Ref* sender, cocos2d::ui::Widget::TouchEventType type){ switch (type){
        ...
        case ui::Widget::TouchEventType::ENDED:{
            ...
            sprintf(positiveUrl,"http://czapi.looper.pro/web/normal_f.php?cid=%s",UserDefault::getInstance()->getStringForKey("caseId").c_str());
            string url=positiveUrl;
            char contraryUrl[1000]={0};//反面
            sprintf(contraryUrl,"http://czapi.looper.pro/web/normal_b.php?cid=%s",UserDefault::getInstance()->getStringForKey("caseId").c_str());
    ...
    cancelBtn->addTouchEventListener([&](Ref* sender, cocos2d::ui::Widget::TouchEventType type){ switch (type){
        ...
        case ui::Widget::TouchEventType::ENDED:{
            ...
            string otherURL=contraryUrl;
            url.append("tz_gjyc_no");
            bodySC->webURL=url;
            otherURL.append("tz_gjyc_no_b");
            bodySC->contraryURL=otherURL;
            Director::getInstance()->pushScene(bodySC);
        }
