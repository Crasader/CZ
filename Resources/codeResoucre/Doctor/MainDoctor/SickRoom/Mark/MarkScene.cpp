    ...
    switch (type)
    {
        ...
        case cocos2d::ui::ListView::EventType::ON_SELECTED_ITEM_END:
        {
            ...
            if (index==0) {
                ...
                auto lenkeSC=LenkeScene::createScene();
                Director::getInstance()->pushScene(lenkeSC);
            }
    ...
    if(this->infoData.HasMember("status")){
        ...
        if (this->infoData["status"].GetInt()==0) {
            ...
            if(val_form.IsObject()){
                ...
                if (!val_form["pf_lenke_result"].IsNull()) {
                    lv->removeChildByTag(3);
                    auto layer4 = createMessageLayout(3,"Lenke分型",val_form["pf_lenke_result"].GetString());
                    lv->insertCustomItem(layer4,3);
                }
