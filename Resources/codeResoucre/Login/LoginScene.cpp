...
//    netManeger->sendMessage("http://api2.innfinityar.com/web/getArtist",CC_CALLBACK_2(LoginScene::onHttpRequestCompleted, this),(char *)buffer.GetString());
    ...
    userName = EditBox::create(Size(400,50), Scale9Sprite::create("alpha.png"));
    ...
    userName->setPlaceholderFontSize(30);
    userName->setFontColor(Color3B::BLACK);
    userName->setPlaceHolder("请输入账户名:");
    userName->setPlaceholderFontColor(Color3B::GRAY);
    userName->setReturnType(EditBox::KeyboardReturnType::DONE);
    userName->setInputMode(cocos2d::ui::EditBox::InputMode::SINGLE_LINE);//可以输入任何，但是不包括换行
    userName->setDelegate(this);
    addChild(userName);
    
    password = EditBox::create(Size(400,50), Scale9Sprite::create("alpha.png"));
    password->setTextHorizontalAlignment(TextHAlignment::LEFT);
    password->setAnchorPoint(Vec2(0, 0));
    password->setPosition(Vec2(130,637));
    password->setFontName("Arial");
    password->setFontSize(30);
    password->setPlaceholderFontSize(30);
    ...
    password->setInputMode(cocos2d::ui::EditBox::InputMode::SINGLE_LINE);//可以输入任何，但是不包括换行
    password->setInputFlag(cocos2d::ui::EditBox::InputFlag::PASSWORD);
    password->setDelegate(this);
    addChild(password);
    
    
/*    password = CursorTextField::getinstance()->textFieldWithPlaceHolder(bkView,"请输入密码","Arial",30);
    password->setPosition(Vec2(130,637));
    password->setInputWidth(visibleSize.width);
    password->setAnchorPoint(Vec2(0, 0));
...
#pragma-EditBoxDelegate
void LoginScene::editBoxReturn(EditBox* editBox)
{
    log("editBox %p was returned !",editBox);
    if (userName == editBox)
    {
        log("userName %s DidEnd !", editBox->getText());
    }
    if (password == editBox)
    {
        log("password %s DidEnd !", editBox->getText());
    }
}
...
//    password->closeIME();
    ...
    if (tag==100) {
        ...
        if (strcmp(userName->getText(), "")==0) {
            ...
            this->addChild(judgeV,10);
            judgeV->runAction(Sequence::create(DelayTime::create(0.5),FadeOut::create(0.5), NULL));
        }else if(strcmp(password->getText(), "")==0){
     ...
     pushDataToNetWork(userName->getText(), password->getText());
        }
    }
}
